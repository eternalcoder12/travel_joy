    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          // 覆盖在底图上的位置标记
          ..._spotLocations.asMap().entries.map((entry) {
            final index = entry.key;
            final location = entry.value;

            // 计算位置标记在屏幕上的位置
            // 这里使用简化的方法，实际应用中需要根据地图比例尺和经纬度计算
            ...
            return Positioned(
              left:
                  MediaQuery.of(context).size.width / 2 + offsetX * zoomFactor,
              top:
                  MediaQuery.of(context).size.height / 2 - offsetY * zoomFactor,
